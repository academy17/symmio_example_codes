...
def main() -> None:
	...
	if (
		...
		raise SystemExit(
			"Missing env vars: RPC_URL, PRIVATE_KEY, DIAMOND_ADDRESS (or SYMMIO_DIAMOND_ADDRESS), TRADE_ID, QUANTITY, PRICE, DEADLINE"
		)
	...
	# sendCloseIntent(tradeId, quantity, price, deadline)
	tx = diamond.functions.sendCloseIntent(
		int(trade_id),
		int(quantity),
		int(price),
		int(deadline),
	...
	print(f"sendCloseIntent tx sent: {h.hex()}")
