...
def main() -> None:
	...
	if not rpc_url or not private_key or not diamond_address or not intent_ids:
		raise SystemExit("Missing env vars: RPC_URL, PRIVATE_KEY, DIAMOND_ADDRESS, INTENT_IDS")
	...
	# cancelCloseIntents(intentIds) - requires CLEARING_HOUSE_ROLE
	tx = diamond.functions.cancelCloseIntents(intent_ids).build_transaction(
		{
			"from": acct.address,
	...
	print(f"cancelCloseIntents tx sent: {h.hex()}")
