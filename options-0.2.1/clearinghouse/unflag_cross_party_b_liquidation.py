...
def main() -> None:
	...
	if not rpc_url or not private_key or not diamond_address or not party_b or not party_a or not collateral:
		raise SystemExit("Missing env vars: RPC_URL, PRIVATE_KEY, DIAMOND_ADDRESS, PARTY_B_ADDRESS, PARTY_A_ADDRESS, COLLATERAL_ADDRESS")
	...
	# unflagCrossPartyBLiquidation(partyB, partyA, collateral) - requires CLEARING_HOUSE_ROLE
	tx = diamond.functions.unflagCrossPartyBLiquidation(
	...
	print(f"unflagCrossPartyBLiquidation tx sent: {h.hex()}")
