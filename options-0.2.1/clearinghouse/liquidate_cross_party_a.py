...
def main() -> None:
	...
	if (
		...
		or not collateral
		or upnl is None
		or collateral_price is None
	):
		raise SystemExit(
			"Missing env vars: RPC_URL, PRIVATE_KEY, DIAMOND_ADDRESS, LIQUIDATION_ID, PARTY_A_ADDRESS, PARTY_B_ADDRESS, COLLATERAL_ADDRESS, UPNL, COLLATERAL_PRICE"
		)
	...
	# liquidateCrossPartyA(liquidationId, partyA, partyB, collateral, upnl, collateralPrice) - requires CLEARING_HOUSE_ROLE
	tx = diamond.functions.liquidateCrossPartyA(
		int(liquidation_id),
	...
	print(f"liquidateCrossPartyA tx sent: {h.hex()}")
