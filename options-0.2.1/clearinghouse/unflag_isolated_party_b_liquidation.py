...
def main() -> None:
	...
	if not rpc_url or not private_key or not diamond_address or not party_b or not collateral:
		raise SystemExit("Missing env vars: RPC_URL, PRIVATE_KEY, DIAMOND_ADDRESS, PARTY_B_ADDRESS, COLLATERAL_ADDRESS")
	...
	# unflagIsolatedPartyBLiquidation(partyB, collateral) - requires CLEARING_HOUSE_ROLE
	tx = diamond.functions.unflagIsolatedPartyBLiquidation(
	...
	print(f"unflagIsolatedPartyBLiquidation tx sent: {h.hex()}")
