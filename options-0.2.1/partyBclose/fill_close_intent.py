...
def main() -> None:
	...
	if (
		...
		raise SystemExit(
			"Missing env vars: RPC_URL, PRIVATE_KEY, DIAMOND_ADDRESS (or SYMMIO_DIAMOND_ADDRESS), INTENT_ID, QUANTITY, PRICE"
		)
	...
	# fillCloseIntent(intentId, quantity, price)
	tx = diamond.functions.fillCloseIntent(
		int(intent_id),
		int(quantity),
		int(price),
	).build_transaction(
	...
	print(f"fillCloseIntent tx sent: {h.hex()}")
