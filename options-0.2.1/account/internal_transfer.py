...
def main() -> None:
	...
	if not rpc_url or not private_key or not diamond_address or not collateral_address or not user_address:
		raise SystemExit("Missing env vars: RPC_URL, PRIVATE_KEY, DIAMOND_ADDRESS, COLLATERAL_ADDRESS, USER_ADDRESS")
	...
	tx = diamond.functions.internalTransfer(
	...
	print(f"internalTransfer tx sent: {tx_hash.hex()}")
