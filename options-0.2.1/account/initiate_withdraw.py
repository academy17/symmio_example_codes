...
def main() -> None:
	...
	if not rpc_url or not private_key or not diamond_address or not collateral_address or not to_address:
		raise SystemExit("Missing env vars: RPC_URL, PRIVATE_KEY, DIAMOND_ADDRESS, COLLATERAL_ADDRESS, TO_ADDRESS")
	...
	# initiateWithdraw(collateral, amount, to)
	tx = diamond.functions.initiateWithdraw(
	...
	print(f"initiateWithdraw tx sent: {tx_hash.hex()}")
