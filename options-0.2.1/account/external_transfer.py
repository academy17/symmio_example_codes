...
def main() -> None:
	...
	if not rpc_url or not private_key or not diamond_address or not collateral_address or not user_address or not target_address:
		raise SystemExit(
			"Missing env vars: RPC_URL, PRIVATE_KEY, DIAMOND_ADDRESS, COLLATERAL_ADDRESS, USER_ADDRESS, TARGET_ADDRESS"
		)
	...
	tx = diamond.functions.externalTransfer(
	...
	print(f"externalTransfer tx sent: {tx_hash.hex()}")
