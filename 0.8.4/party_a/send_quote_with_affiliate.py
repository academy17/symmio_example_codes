...
class SendQuoteWithAffiliateClient:
    ...
    def extract_quote_id(self, receipt) -> Optional[int]:
        print("Log decoding skipped.")
        return None
    
    def send_quote_with_affiliate(self) -> int:
        """Execute sendQuoteWithAffiliate function and return the quote ID"""
        try:
            # 1. Fetch market info
            ...
            txn = self.diamond.functions.sendQuoteWithAffiliate(
                party_bs_white_list,
                market["id"],
                self.config["position_type"],
