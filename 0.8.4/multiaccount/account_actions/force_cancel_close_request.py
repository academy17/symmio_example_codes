...
class MultiAccountForceCancelCloseRequestClient:
    ...
    def force_cancel_close_request_via_multiaccount(self, quote_id: int):
        """Force cancel a close request via MultiAccount"""
        try:
            # Build the forceCancelCloseRequest transaction
            force_cancel_close_txn = self.diamond.functions.forceCancelCloseRequest(
                quote_id
            ).build_transaction({
            ...
            encoded_force_cancel_close = force_cancel_close_txn["data"]
            ...
            print(f"Error forcing to cancel close request: {e}")
            raise

def main():
    """Main function to demonstrate forcing to cancel a close request via MultiAccount"""
    client = MultiAccountForceCancelCloseRequestClient(CONFIG)
    
    # Replace with the actual quote ID
    quote_id = 2221
    
    print(f"Force canceling close request for quote ID: {quote_id}")
    receipt = client.force_cancel_close_request_via_multiaccount(quote_id)
    print(f"Force cancel close request transaction receipt: {receipt}")
