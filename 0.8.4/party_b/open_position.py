...
class OpenPositionClient:
    ...
    def open_position(self, quote_id: int, filled_amount: int, opened_price: int, pair_upnl_sig):
        """Open a position by providing a valid PairUpnlAndPriceSig"""
        try:
            # Build transaction
            txn = self.diamond.functions.openPosition(
                quote_id,
                filled_amount,
                opened_price,
                pair_upnl_sig
...
def main():
    """Main function to demonstrate opening a position"""
    client = OpenPositionClient(CONFIG)
    
    # Example: Parameters for opening a position
    quote_id = 1  # Replace with the actual quote ID
    filled_amount = Web3.to_wei(1, "ether")  # Replace with the filled amount
    opened_price = Web3.to_wei(2000, "ether")  # Replace with the opened price
    ...
    # Open the position
    print(f"Opening position for quote ID: {quote_id}")
    receipt = client.open_position(quote_id, filled_amount, opened_price, pair_upnl_sig)
    print(f"Open position transaction receipt: {receipt}")
