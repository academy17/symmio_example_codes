...
class FillCloseRequestClient:
    ...
    def fill_close_request(self, quote_id: int, filled_amount: int, closed_price: int, pair_upnl_sig):
        """Fill a close request by providing a valid PairUpnlAndPriceSig"""
        try:
            # Build transaction
            txn = self.diamond.functions.fillCloseRequest(
                quote_id,
                filled_amount,
                closed_price,
                pair_upnl_sig
            ...
            print(f"Error filling close request: {e}")
            raise

def main():
    """Main function to demonstrate filling a close request"""
    client = FillCloseRequestClient(CONFIG)
    
    # Example: Parameters for filling a close request
    quote_id = 2213  # Replace with the actual quote ID
    filled_amount = Web3.to_wei(6, "ether")  # Replace with the filled amount
    closed_price = Web3.to_wei(3.3, "ether")  # Replace with the closed price
    ...
    # Fill the close request
    print(f"Filling close request for quote ID: {quote_id}")
    receipt = client.fill_close_request(quote_id, filled_amount, closed_price, pair_upnl_sig)
    print(f"Fill close request transaction receipt: {receipt}")
