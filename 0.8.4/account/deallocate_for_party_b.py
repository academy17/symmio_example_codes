...
# Set these as needed
PARTY_B = account.address  # The Party B (msg.sender)
PARTY_A = "0xEb42F3b1aC3b1552138C7D30E9f4e0eF43229542"  # Replace with actual Party A address
...
def main():
    try:
        print(f"Fetching uPnl signature for Party B: {PARTY_B} and Party A: {PARTY_A}")
        result = fetch_upnl_signature(PARTY_B, PARTY_A, CHAIN_ID, DIAMOND_ADDRESS)
        upnl_sig = format_upnl_signature(result)
        print("Formatted uPnl signature:")
        pretty_print_upnl_sig(upnl_sig)

        # Build and send deallocateForPartyB transaction
        txn = diamond.functions.deallocateForPartyB(AMOUNT, PARTY_A, upnl_sig).build_transaction({
        ...
        print(f"deallocateForPartyB transaction sent! Tx hash: {tx_hash.hex()}")
