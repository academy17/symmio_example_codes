...
CHAIN_ID = os.getenv("CHAIN_ID", "137")  # Default to Polygon
...
def fetch_upnl_signature(party_a_address, chain_id, symmio_address):
    """Fetch uPnl signature from Muon API"""
    url = f"{MUON_BASE_URL}?app=symmio&method=uPnl_A&params[partyA]={party_a_address}&params[chainId]={chain_id}&params[symmio]={symmio_address}"
...
def main():
    try:
        ...
        print(f"Fetching uPnl signature for Party A: {party_a_address}")
        result = fetch_upnl_signature(party_a_address, CHAIN_ID, symmio_address)
        
        upnl_sig = format_upnl_signature(result)
        print("Formatted uPnl signature:")
        
        pretty_print_upnl_sig(upnl_sig)        
        # Build and send deallocate transaction
        deallocate_txn = diamond.functions.deallocate(AMOUNT, upnl_sig).build_transaction({
        ...
        signed_deallocate = w3.eth.account.sign_transaction(deallocate_txn, private_key=PRIVATE_KEY)
        deallocate_tx_hash = w3.eth.send_raw_transaction(signed_deallocate.raw_transaction)
        print(f"Deallocate transaction sent! Tx hash: {deallocate_tx_hash.hex()}")
