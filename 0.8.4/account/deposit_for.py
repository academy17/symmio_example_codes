...
def main():
    try:
        ...
        # 2. Deposit for user
        deposit_txn = diamond.functions.depositFor(USER, AMOUNT).build_transaction({
            "from": account.address,
            "nonce": w3.eth.get_transaction_count(account.address),
            "gas": 200000,
            "gasPrice": w3.eth.gas_price,
        })
        signed_deposit = w3.eth.account.sign_transaction(deposit_txn, private_key=PRIVATE_KEY)
        deposit_tx_hash = w3.eth.send_raw_transaction(signed_deposit.raw_transaction)
        print(f"DepositFor transaction sent! Tx hash: {deposit_tx_hash.hex()}")
