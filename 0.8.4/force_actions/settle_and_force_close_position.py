...
# ------------------ HighLowPriceSig Helper Functions ------------------
...
# ------------------ SettlementSig Helper Functions ------------------

def pretty_print_settlement_sig(settlement_sig):
    req_id, timestamp, quotes_settlements_data, upnl_party_bs, upnl_party_a, gateway_signature, schnorr_sign = settlement_sig
    signature, owner, nonce = schnorr_sign
    
    pretty = {
        "reqId": req_id.hex(),
        "timestamp": timestamp,
        "quotesSettlementsData": [list(q) for q in quotes_settlements_data],
        "upnlPartyBs": [str(u) for u in upnl_party_bs],
        "upnlPartyA": upnl_party_a,
        "gatewaySignature": gateway_signature.hex(),
        "sigs": {
            "signature": str(signature),
...
def fetch_settlement_signature(party_a, chain_id, symmio_address, quote_ids):
    """Fetch settlement signature from Muon API"""
    quote_ids_str = json.dumps(quote_ids)
    url = (
        f"{MUON_BASE_URL}?app=symmio&method=settle_upnl"
        f"&params[partyA]={party_a}"
        f"&params[chainId]={chain_id}"
        f"&params[symmio]={symmio_address}"
...
def format_settlement_signature(result):
    """Format the API response into SettlementSig structure as a tuple for web3.py"""
    req_id = result["result"]["reqId"]
    timestamp = int(result["result"]["data"]["timestamp"])
    quotes_data = result["result"]["data"]["result"]["quotesSettlementsData"]
    upnl_party_bs = [int(u) for u in result["result"]["data"]["result"]["upnlPartyBs"]]
    upnl_party_a = int(result["result"]["data"]["result"]["upnlPartyA"])
    gateway_signature = result["result"]["nodeSignature"]
    
    # Format quotes settlement data as tuples
    quotes_settlements_data = []
    for quote in quotes_data:
        quote_tuple = (
            int(quote["quoteId"]),
            int(quote["openedPrice"]),
            int(quote["closedPrice"]),
            int(quote["quantity"]),
            int(quote["closedAmount"]),
            int(quote["fundingFee"])
        )
        quotes_settlements_data.append(quote_tuple)
    
    # Extract updated prices (needed for the settleAndForceClosePosition function)
    updated_prices = []
    for quote in quotes_data:
        # The updated price is in the openedPrice field of each quote
        updated_prices.append(int(quote["openedPrice"]))
    ...
    # Return as tuple for SettlementSig
    schnorr_sign = (signature, owner, nonce)
    settlement_sig = (
        req_id_bytes, timestamp, quotes_settlements_data, upnl_party_bs, 
        upnl_party_a, gateway_signature_bytes, schnorr_sign
    )
    
    return settlement_sig, updated_prices
...
def main():
    try:
        ...
        # 1. Fetch price range signature (HighLowPriceSig)
        ...
        # 
        price_range_result = fetch_price_range_signature(
            party_a_address, party_b_address, T0, T1, 
            SYMBOL_ID, CHAIN_ID, symmio_address
        )
        
        high_low_price_sig = format_price_range_signature(price_range_result)
        print("\nFormatted price range signature:")
        pretty_print_price_sig(high_low_price_sig)
        
        # 2. Fetch settlement signature (SettlementSig)
        print(f"\nFetching settlement signature for:")
        print(f"- Party A: {party_a_address}")
        print(f"- Quote IDs: [{QUOTE_ID}]")
        
        settlement_result = fetch_settlement_signature(
            party_a_address, CHAIN_ID, symmio_address, [QUOTE_ID]
        )
        
        settlement_sig, updated_prices = format_settlement_signature(settlement_result)
        print("\nFormatted settlement signature:")
        pretty_print_settlement_sig(settlement_sig)
        print("\nUpdated Prices:")
        print(updated_prices)
        
        # 3. Build and send settleAndForceClosePosition transaction
        settle_force_close_txn = diamond.functions.settleAndForceClosePosition(
            QUOTE_ID, high_low_price_sig, settlement_sig, updated_prices
        ).build_transaction({
            "from": account.address,
            "nonce": w3.eth.get_transaction_count(account.address),
            "gas": 800000,  # Adjust gas as needed - this function needs more gas
            "gasPrice": w3.eth.gas_price,
        })
        
        signed_txn = w3.eth.account.sign_transaction(settle_force_close_txn, private_key=PRIVATE_KEY)
        tx_hash = w3.eth.send_raw_transaction(signed_txn.raw_transaction)
        print(f"\nsettleAndForceClosePosition transaction sent! Tx hash: {tx_hash.hex()}")
