"""PerpsHub Majors Demo: Instant Close

Run (interactive prompts)
- python trading_bot_example/majors_perpshub_close_position_demo.py
...
- MAJORS_SYMBOL_ID (used for Muon close_price)
- MAJORS_CLOSE_SLIPPAGE
"""
...
def get_nonce(perpshub_base_url: str, party_a: str) -> str:
...
def login(*, perpshub_base_url: str, chain_id: int, active_account: str, wallet: Account) -> str:
	...
	nonce = get_nonce(perpshub_base_url, active_account)
...
def fetch_muon_price(*, muon_base_url: str, party_a: str, chain_id: int, symmio: str, symbol_id: int) -> Decimal:
	url = get_muon_url(muon_base_url=muon_base_url, party_a=party_a, chain_id=chain_id, symmio=symmio, symbol_id=symbol_id)
	print(f"[MUON] {url}")
	data = http_get_json(url)
...
def fetch_symbol_info(perpshub_base_url: str, symbol_id: int) -> dict:
...
def instant_close(
	*,
	perpshub_base_url: str,
	token: str,
	quote_id: int,
	quantity_to_close: str,
	close_price: str,
) -> dict:
	url = f"{perpshub_base_url}instant_close"
	payload = {
	...
	print(f"[CLOSE] Status={resp.status_code} Body={resp.text}")
	resp.raise_for_status()
...
def main() -> None:
	...
	parser = argparse.ArgumentParser(description="Majors (PerpsHub) instant close demo")
	...
	parser.add_argument("--symbol-id", type=int, default=int(os.getenv("MAJORS_SYMBOL_ID", "4")))
	parser.add_argument(
		"--slippage",
		default=os.getenv("MAJORS_CLOSE_SLIPPAGE", "0.01"),
		help="Slippage as fraction (e.g. 0.01 for 1%%). Close long decreases; close short increases.",
	)
	# PerpsHub instant_close is always MARKET and does not accept deadline/order_type.
	parser.add_argument("--prompt", action="store_true", help="Prompt for any missing inputs")
	parser.add_argument("--no-prompt", action="store_true", help="Disable prompting; error if required inputs are missing")
	args = parser.parse_args()

	# Default behavior: prompt unless explicitly disabled.
	...
	if should_prompt:
		...
		args.slippage = prompt_str("Slippage (fraction, e.g. 0.01)", str(args.slippage))
	else:
		missing = [
			name
			for name, val in (
	...
	majors_subaccount = os.getenv("MAJORS_SUBACCOUNT")
	fallback_subaccount = os.getenv("SUB_ACCOUNT_ADDRESS")
	active_account_raw = majors_subaccount or fallback_subaccount
	if not active_account_raw:
		raise ValueError("Missing subaccount address. Set MAJORS_SUBACCOUNT (preferred) or SUB_ACCOUNT_ADDRESS.")
	...
	symmio_diamond = os.getenv(
		"PERPSHUB_SYMMIO_DIAMOND_ADDRESS",
		os.getenv("VIBE_DIAMOND_ADDRESS", "0x91Cf2D8Ed503EC52768999aA6D8DBeA6e52dbe43"),
	)
	symmio_diamond = Web3.to_checksum_address(symmio_diamond)

	wallet = Account.from_key(private_key)

	print("=" * 60)
	print("Majors (PerpsHub) Demo: Login -> Instant Close")
	print("=" * 60)
	...
	print(
		f"Quote ID: {args.quote_id} | Qty: {args.quantity} | PositionType: {args.position_type} | SymbolId: {args.symbol_id}"
	)
	print("=" * 60)

	token = login(perpshub_base_url=perpshub_base_url, chain_id=chain_id, active_account=active_account, wallet=wallet)
	print("✅ Logged in")

	symbol_info = fetch_symbol_info(perpshub_base_url, int(args.symbol_id))
	...
	instant_close(
		perpshub_base_url=perpshub_base_url,
		token=token,
